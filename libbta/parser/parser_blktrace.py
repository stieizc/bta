...
GAPS=["读／写", "位置＋长度", "A2Q", "Q2G", "Q2M", "G2I", "I2D", "D2C"]
FINAL_ACTIONS = "MC"
...
class record:
    ...
    def get_field(self, field, start=0):
        if self.fields.__contains__(field):
            return self.fields[field] - start
        else:
            return -1
    def str_field(self, field, start=0):
    ...
    def to_list(self, start=0):
        ...
        return l
    def gap_list(self):
        #GAPS=["A2Q", "Q2G", "Q2M", "G2I", "I2D", "D2C"]
        l = [ "{0:4}".format(self.RWBS) ] + [ str(self.blocks) ]
        if self.has_field('M'):
            l += [
                    str(self.get_field('Q')-self.get_field('A')),
                    '',
                    str(self.get_field('M')-self.get_field('Q')),
                    '',
                    '',
                    ''
                    ]
            ...
            l += [ 
                    str(self.get_field('Q')-self.get_field('A')),
                    str(self.get_field('G')-self.get_field('Q')),
                    '',
                    str(self.get_field('I')-self.get_field('G')), 
                    str(self.get_field('D')-self.get_field('I')),
                    str(self.get_field('C')-self.get_field('G')),
                ]
        return l
...
class table:
    ...
    def read_records(self, fd=sys.stdin, global_offset=0, time_offset=0):
    ...
    def filter(self, ranges):
        for r in self.records:
            if not ranges.find_block(r.blocks.offset):
                print("Block filtered: ", r)
    ...
    def gen_r2r_maps(self, htable):
        ...
        for m in self.maps:
            ...
            try:
                ...
                print(record, addr, file=sys.stderr)
